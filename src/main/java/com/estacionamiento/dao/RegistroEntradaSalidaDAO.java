...
public class RegistroEntradaSalidaDAO {
    ...
    public boolean crear(RegistroEntradaSalida registro) {
        ...
        try (PreparedStatement pstmt = conexion.prepareStatement(sql)) {
            ...
            System.out.println("Vehiculo ID: " + registro.getVehiculoId());
            System.out.println("Cajon ID: " + registro.getCajonId());
            System.out.println("Fecha Entrada: " + registro.getFechaEntrada());
            System.out.println("Estado: " + registro.getEstado());
            System.out.println("Promocion: " + registro.getPromocionAplicada());
            System.out.println("Estacionamiento ID: " + registro.getEstacionamientoId());
            ...
            pstmt.setTimestamp(
                3,
                Timestamp.valueOf(registro.getFechaEntrada())
            );

            pstmt.setString(4, registro.getEstado());

            // Permitir NULL correctamente
            ...
            System.err.println("=== ERROR SQL AL CREAR REGISTRO ===");
    ...
    public RegistroEntradaSalida obtenerPorId(int id) {
        ...
        String sql =
            "SELECT * FROM registros_entrada_salida WHERE id = ?";
    ...
    public RegistroEntradaSalida obtenerActivoDelVehiculo(int vehiculoId) {
        ...
        String sql =
            ...
            "WHERE vehiculo_id = ? AND estado = 'Activo' LIMIT 1";
    ...
    public boolean finalizarRegistro(
        ...
        String sql =
            ...
            "promocion_aplicada = ?, estado = 'Finalizado' " +
            "WHERE id = ?";
        ...
        try (PreparedStatement pstmt = conexion.prepareStatement(sql)) {
            ...
            System.err.println(
                "Error al finalizar registro: " + e.getMessage()
            );
    ...
    public List<RegistroEntradaSalida> obtenerPorEstacionamiento(
    ...
    public boolean actualizar(RegistroEntradaSalida registro) {
        ...
        String sql =
            "UPDATE registros_entrada_salida " +
            "SET vehiculo_id = ?, cajon_id = ?, " +
            "fecha_entrada = ?, fecha_salida = ?, monto = ?, " +
            "promocion_aplicada = ?, estado = ? " +
            "WHERE id = ?";
        ...
        try (PreparedStatement pstmt = conexion.prepareStatement(sql)) {
            ...
            if (registro.getFechaSalida() != null) {
                ...
                pstmt.setTimestamp(
                    4,
                    Timestamp.valueOf(registro.getFechaSalida())
                );
    ...
    public boolean eliminar(int id) {
        ...
        String sql =
            "DELETE FROM registros_entrada_salida WHERE id = ?";
    ...
    public double obtenerIngresoDelDia(
        ...
        try (PreparedStatement pstmt = conexion.prepareStatement(sql)) {
            ...
            System.err.println(
                "Error al obtener ingreso: " + e.getMessage()
            );
        }
    ...
    private RegistroEntradaSalida mapearResultSet(ResultSet rs)
            throws SQLException {
        ...
        if (fechaEntrada != null) {
            registro.setFechaEntrada(
                fechaEntrada.toLocalDateTime()
            );
        }
        ...
        if (fechaSalida != null) {
            registro.setFechaSalida(
                fechaSalida.toLocalDateTime()
            );
        }

        registro.setMonto(rs.getDouble("monto"));

        registro.setPromocionAplicada(
            rs.getString("promocion_aplicada")
        );

        registro.setEstado(rs.getString("estado"));

        registro.setEstacionamientoId(
            rs.getInt("estacionamiento_id")
        );
